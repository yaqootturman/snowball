...
const UserPledge = ({ history, userPledge: { pledge_id, title, description, number_of_enrollement, banner_img } }) => {
  return (
    <div className="user-pledge" onClick={() => { history.push(`/action-category/pledge/${title}`, { pledge_id: pledge_id }) }}>
      <img className="user-pledge__pledge-image" alt="user pledge" src={banner_img}  />
